...
export default function ChatAI() {
  ...
  const handleSendMessage = async (text) => {
    ...
    try {
      ...
      const { data } = await aiGuideApi.ask({ question: text, topic: 'museum' });
      const aiResponse = {
        id: Date.now() + 1,
        role: 'ai',
        content: data.answer || data.response || 'I could not find an answer for that question.',
      };
      ...
      const errorResponse = {
        id: Date.now() + 1,
        role: 'ai',
        content: getApiErrorMessage(error, 'The AI Guide is unavailable right now. Please try again.'),
      };
